...
const App: React.FC = () => {
  ...
  const createNewProject = async () => {
    ...
    try {
      ...
      const newProjectPath = path.join(dir, projectName);
      try {
        ...
        await window.electron?.ipcRenderer.invoke('fs:createDirectory', path.join(newProjectPath, 'src'));
        await window.electron?.ipcRenderer.invoke('fs:createDirectory', path.join(newProjectPath, 'assets'));
        await window.electron?.ipcRenderer.invoke('fs:writeFile', path.join(newProjectPath, 'README.md'), `# ${projectName}\n\nA new project created with LSEditor.`);
